...
class WOOCOS_Email extends WC_Email {
    /**
     * Stores the custom order status object passed in constructor
     * so we can use it later (e.g. in preview context).
     */
    protected $woocos_order_status;
    ...
    public function __construct($order_status) {
            ...
            // Keep the original order status object for later usage (e.g. preview)
            $this->woocos_order_status = $order_status;
    ...
    function woocos_trigger( $order_id , $custom_order_status) {
        ...
        $email_data = get_option('woocommerce_' . $custom_order_status->slug . '_settings');
    ...
    public function woocos_get_attachments($custom_order_status, $order_id)
    {
        ...
        return apply_filters( 'woocommerce_email_attachments', array(), $custom_order_status->slug, $order, $this );
    }
    ...
    /**
     * Ensure WooCommerce preview uses our renderer with real order data.
     */
    public function get_content_html() {
        // Try to use existing object first
        $order_id = ( $this->object && isset( $this->object->order_id ) ) ? $this->object->order_id : 0;

        if ( ! $order_id ) {
            // Fallback to the most recent order ID for preview purposes
            $orders = function_exists('wc_get_orders') ? wc_get_orders( array( 'limit' => 1, 'orderby' => 'date', 'order' => 'DESC', 'return' => 'ids' ) ) : array();
            if ( ! empty( $orders ) ) {
                $order_id = $orders[0];
            }
        }

        // Load email settings for this specific email id
        $email_data = get_option( 'woocommerce_' . $this->id . '_settings' );
        ...
        if ( $order_id ) {
            // Build the object and render via our custom renderer
            $this->object = $this->woocos_create_object( $order_id, $email_data );
            return $this->woocos_get_content_html( $email_data, $this->woocos_order_status );
        }

        // As a very last resort, let parent try to render something minimal
        return parent::get_content_html();
    }

    /**
     * Basic plain text fallback for completeness.
     */
