...
class WOOCOS_Email_Manager {
     ...
     public function __construct()
     {
		...
		// Allow previewing via type=WOOCOS_Email by mapping to the first custom email
		add_action( 'admin_init', array( $this, 'woocos_fix_preview_type' ) );
     }
	...
	// Map preview type=WOOCOS_Email to a concrete registered email id (e.g. {slug})
	public function woocos_fix_preview_type() {
		if ( ! is_admin() ) {
			return;
		}
		if ( empty( $_GET['preview_woocommerce_mail'] ) || empty( $_GET['type'] ) ) {
			return;
		}
		if ( $_GET['type'] !== 'WOOCOS_Email' ) {
			return;
		}
		...
		$first = reset( $custom_order_statuses );
		if ( ! $first || empty( $first->slug ) ) {
			return;
		}
